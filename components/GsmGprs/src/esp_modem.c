...
static void esp_handle_uart_data(esp_modem_dte_t *esp_dte)
{
    ...
    if (length) {
        ...
        ESP_LOGI(MODEM_TAG, "new data rec =  %s", esp_dte->buffer);
        // char tempbuf[50];

        // strcpy(tempbuf, esp_dte->buffer);

        // if(!strcmp(tempbuf, "+PPPD: DISCONNECTED"))
        // {
        //     ESP_LOGI(MODEM_TAG,"NO internet connection");
        // }
    }
}
...
modem_dte_t *esp_modem_dte_init(const esp_modem_dte_config_t *config)
{
    ...
    uart_config_t uart_config = {
        .baud_rate = config->baud_rate,//115200,//config->baud_rate,
    ...
    if (!uart_is_driver_installed(esp_dte->uart_port))
    {
