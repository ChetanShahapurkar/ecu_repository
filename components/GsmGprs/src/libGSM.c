...
static const char *TAG = "LibGsm";

char new_sms[150];
char sms_sender[15];
bool sms_decoded = false;

char sms_sender_num_string[15];
...
void get_sms_at_loc(int num)
{
	int size = 512;
	char *rbuffer = malloc(size);
	int res = atCmd_waitResponse("AT+CMGR=1\r\n", NULL, NULL, -1, 2000, &rbuffer, size);
	ESP_LOGI(TAG, "res is : %d buffer is : %s", res , rbuffer);
}
...
void smsRead(SMS_Messages *SMSmesg, int sort)
{
	...
	ESP_LOGI(TAG, "SMS BUFFER is : %d buffer is : %s", res , rbuffer);
...
int smsDeleteAll(void)
{
	...
	char buf[64];
	sprintf(buf,"AT+CMGD=1,4\r\n");

	return atCmd_waitResponse(buf, GSM_OK_Str, NULL, -1, 5000, NULL, 0);
}
void LibGSM__CNMIDisable()
{

	char buf[64];
	sprintf(buf,"AT+CNMI=0,0,0,0,0\r");
	return atCmd_waitResponse(buf, GSM_OK_Str, NULL, -1, 5000, NULL, 0);

}

void LibGSM__CNMIEnable()
{
...
bool libGSM__GetSmSDecodestatus()
{
	return sms_decoded;
}

void libGSM__SetSmSDecodestatus(bool status)
{
	sms_decoded = status;
}

bool sms_ready_to_decode = false;

bool libGSM__GetSmsReadyToDecodeStatus()
{
	return sms_ready_to_decode;
}

void libGSM__SetSmsReadyToDecodeStatus(bool state)
{
	sms_ready_to_decode  = state;
}

void ReadNewSms(void)
{
	sms_ready_to_decode = false;
	...
	char buf[160];
        ...
        if (messages.nmsg) {
			printf("\r\nReceived messages: %d\r\n", messages.nmsg);
			SMS_Msg *msg;
			// for (int i=0; i<messages.nmsg; i++) { //read messages at all locations
			for (int i=0; i<1; i++) { // only read message at 0 location
                ...
                // new_sms = msg->msg;
                // sms_sender_num = msg->from;
				// sms_decoded = true;

				printf("LibGSM message from : %s", msg->from);
				printf("LibGSM message body : %s", msg->msg);

				sms_ready_to_decode = true;
			...
			memset(new_sms, '\0', sizeof(new_sms));
			memset(sms_sender, '\0', sizeof(sms_sender));

			strcpy(new_sms,msg->msg);
			strcpy(sms_sender,msg->from);

			const char *i1_1 = strstr(sms_sender, "+91");

			if(i1_1 == NULL)
			{
				char sms_sender_out_temp[10] = "";
				char sms_sender_out[50] = "" ;

				int num = 0;
				
				for(int x = 0; x < strlen(sms_sender); x++)
				{

					char xyz = sms_sender[x];
					x = x+1;
					char abc = sms_sender[x];
					...
					printf("%d\n",num);
					sprintf(sms_sender_out_temp,"%c", num);
					strcat(sms_sender_out,sms_sender_out_temp);
				...
				printf("%s",sms_sender_out);
				ESP_LOGI(TAG, "NEW Sender Num = %s ", sms_sender_out);

				strcpy(sms_sender,sms_sender_out);
				strcpy(new_sms,msg->msg);
...
bool LibGSM__GetSMS( char **buf, char **sender)
{
	bool state = false;
	...
	if(sms_decoded == true)
	{
		...
		*buf = new_sms;
		*sender = sms_sender;
		memset(sms_sender_num_string, '\0', sizeof(sms_sender_num_string));
		strcpy(sms_sender_num_string, sms_sender);
		printf("Libgsm sender = %s",sms_sender);
...
void gsm_escapePPP(void)
{
	...
	char buf[64] = {'\0'};
	char *pbuf = buf;
	int res = atCmd_waitResponse("+++\r\n", NULL, NULL, -1, 5000, &pbuf, 63);
	ESP_LOGI(TAG," +++ repsonse   :%s", pbuf);
	
}

const char *LibGsm__getNum()
{
	// return sms_sender;
	return sms_sender_num_string;
}

void gsm_TurnOnGPS()
{
	// char buf1[64];
	// sprintf(buf1,"AT+CGPS=1\r\n"); //Turn On gps
	// atCmd_waitResponse(buf1, GSM_OK_Str, NULL, -1, 5000, NULL, 0);

	char buf1[64];
	uint8_t f = 1;
	char buf[64] = {'\0'};
	char *pbuf = buf;
	int res = atCmd_waitResponse("AT+CGPS=1,1\r\n", NULL, NULL, -1, 5000, &pbuf, 63);
	ESP_LOGI(TAG,"GPS ON OFF  :%s", pbuf);
...
{
	char buf1[64];
	sprintf(buf1,"AT+CGPS=0\r\n"); //Turn On gps
	atCmd_waitResponse(buf1, GSM_OK_Str, NULL, -1, 5000, NULL, 0);
}


void gsm_getLocation()
{
	...
	char buf[64] = {'\0'};
	char *pbuf = buf;
	int res = atCmd_waitResponse("AT+CGPSINFO\r\n", NULL, NULL, -1, 5000, &pbuf, 63);
	ESP_LOGI(TAG,"GPS LOC  :%s", pbuf);
}

void gsm_NetworkTimeSetup()
{
	...
	char *pbuf = buf;
	int res = atCmd_waitResponse("AT\r\n", NULL, NULL, -1, 2000, &pbuf, 63);
	ESP_LOGI(TAG,"AT BUFFER :%s", pbuf);

	res = atCmd_waitResponse("ATE0\r\n", NULL, NULL, -1, 2000, &pbuf, 63);
	ESP_LOGI(TAG,"ATE0 BUFFER :%s", pbuf);

	// res = atCmd_waitResponse("AT+QNITZ=1\r\n", NULL, NULL, -1, 2000, &pbuf, 63);
	// ESP_LOGI(TAG,"TIME AT+QNITZ=1 :%s", pbuf);

	res = atCmd_waitResponse("AT+CTZU=1\r\n", NULL, NULL, -1, 2000, &pbuf, 63);
	ESP_LOGI(TAG,"TIME AT+CTZU=1 :%s", pbuf);

	res = atCmd_waitResponse("AT+CCLK?\r\n", NULL, NULL, -1, 2000, &pbuf, 63);
	ESP_LOGI(TAG,"TIME AT+CCLK? :%s", pbuf);

	// res = atCmd_waitResponse("AT+CLTS=1\r\n", NULL, NULL, -1, 2000, &pbuf, 63);
	// ESP_LOGI(TAG,"TIME AT+CLTS=1 :%s", pbuf);

	res = atCmd_waitResponse("AT&W\r", NULL, NULL, -1, 2000, &pbuf, 63);
	ESP_LOGI(TAG,"TIME AT&W :%s", pbuf);
	

	// int res = atCmd_waitResponse("AT\r\n", NULL, NULL, -1, 2000, &pbuf, 63);
	// ESP_LOGI(TAG,"TIME BUFFER :%s", pbuf);
	// sprintf(buf1,"AT\r\n"); 
	// atCmd_waitResponse(buf1, GSM_OK_Str, NULL, -1, 5000, NULL, 0);
	// sprintf(buf1,"AT0\r\n"); 
	// atCmd_waitResponse(buf1, GSM_OK_Str, NULL, -1, 5000, NULL, 0);
	// sprintf(buf1,"AT+QNITZ=1\r\n"); 
	// atCmd_waitResponse(buf1, GSM_OK_Str, NULL, -1, 5000, NULL, 0);
	// sprintf(buf1,"AT+CTZU=1\r\n");
	// atCmd_waitResponse(buf1, GSM_OK_Str, NULL, -1, 5000, NULL, 0);
	// sprintf(buf1,"AT+CLTS=1\r");
	// atCmd_waitResponse(buf1, GSM_OK_Str, NULL, -1, 5000, NULL, 0);
	// sprintf(buf1,"AT+CFUN=1,1\r\n");
	// atCmd_waitResponse(buf1, GSM_OK_Str, NULL, -1, 5000, NULL, 0);
	ESP_LOGI(TAG,"GSM Time setup done..Resetting GSM modem");

}

void gsm_getTime(char **buff)
{
...
void gsm_readTime(void)
{
	...
	char *pbuf = buf;
	int res = atCmd_waitResponse("AT+CCLK?\r\n", NULL, NULL, -1, 2000, &pbuf, 63);
	sprintf(time_buff, "%s",pbuf);

	ESP_LOGI(TAG,"TIME BUFFER :%s", time_buff);

}

char *gsm__getTimeBufferVal()
{
	return  time_buff;
}

void gsm_checkSimPresense(char **buff)
{
	char buf1[64];
	...
	char buf[64] = {'\0'};
	char *pbuf = buf;
	int res = atCmd_waitResponse("AT+CSMINS?\r\n", NULL, NULL, -1, 5000, &pbuf, 63);
	ESP_LOGI(TAG,"SIM presense BUFFER :%s", pbuf);
	*buff = pbuf;
