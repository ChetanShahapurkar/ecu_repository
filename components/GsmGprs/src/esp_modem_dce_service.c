...
esp_err_t esp_modem_dce_get_imei_number(modem_dce_t *dce)
{
    ...
    // gprsMQTT__setimesstatus(true);
    return ESP_OK;
    
err:
    // gprsMQTT__setimesstatus(false);
    return ESP_FAIL;
}
...
esp_err_t esp_modem_dce_get_imsi_number(modem_dce_t *dce)
{
    ...
    DCE_CHECK(dce->state == MODEM_STATE_SUCCESS, "get imsi number failed", err_imsi);
    ESP_LOGD(DCE_TAG, "get imsi number ok");
    return ESP_OK;
err:
    return ESP_FAIL;
err_imsi:
    dce->imsi_sucess = false;
    // return ESP_FAIL; esp32 will reset will fail is reported
    return ESP_OK;
}
...
esp_err_t esp_modem_dce_get_operator_name(modem_dce_t *dce)
{
    ...
    return ESP_OK;
err:
    // return ESP_FAIL; Esp32 will reset if fail is reported
    return ESP_OK;
}
